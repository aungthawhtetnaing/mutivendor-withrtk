...
function createData(name, code,short, population,validity, size) {
  const density = population / size;
  return { name, code,short, population,validity, size };
}
...
export default function AllCoupon() {
  ...
  const columns = [
    ...
    { id: 'code', label: 'Coupon Name', minWidth: 100 },
    { id: 'short', label: 'Coupon Discount', minWidth: 100 
    ...
    },
    {
      ...
      align: 'center'
      , format: (value) =><div>
      {currentDate > new Date(value) ?<Chip label='invalid' color='error'/>  : <Chip label='valid' color='success'/>}
</div>,
    },
    {
      id: 'size',
      ...
      align: 'center',
      format: (id) => <div>
                            <Link to={`/dashboard/editcoupon/${id}`} style={{textDecoration:'none',color:"black"}} >
                              <Button 
  ...
  function handleDelete(id){
    ...
    .then(willDelete => {
      if (willDelete) {
        fetch(`http://192.168.2.106:9999/api/admin/coupon/${id}`,{
          method:'DELETE'
...
const rows = Coupon?.map(banner => createData(banner.id, banner.coupon_name,banner.coupon_discount,banner.coupon_validity,banner.coupon_validity));
                   ...
                   {columns.map((column) => {
                      ...
                      return (
                        <TableCell key={column.id} align={column.align}>
                          {column.format ? 
                            (column.id === 'short' ? column.format(row.short) :
                          ...
                          {/* {column.format ? column.format(row.short) : value} */}
                          {/* {column.format ? column.format(row.validity) : value} */}
                        </TableCell>
                      );
                    })}
                  </TableRow>
