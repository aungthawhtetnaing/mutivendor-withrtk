...
export default function AllBanner() {
  ...
  const columns = [
    ...
    { id: 'code', label: 'Banner Title', minWidth: 100 },
    { id: 'short', label: 'Banner Url', minWidth: 100 },
    ...
    },
    {
      ...
      align: 'center',
      format: (id) => <div>
                            <Link to={`/dashboard/editbanner/${id}`} style={{textDecoration:'none',color:"black"}} >
                              <Button 
  ...
  function handleDelete(id){
    ...
    .then(willDelete => {
      if (willDelete) {
        fetch(`http://192.168.2.106:9999/api/admin/banner/${id}`,{
          method:'DELETE'
...
const Banner=useSelector(state=>state.allBanner.banner)
...
const rows = Banner?.map(banner => createData(banner.id, banner.banner_title,banner.banner_url, banner.banner_image ));
