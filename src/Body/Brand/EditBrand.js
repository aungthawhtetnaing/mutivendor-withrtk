...
const EditBrand = () => {
  ...
  console.log("EditallBrand",allBrand)
  
  useEffect(()=>{
    dispatch(allBrandData())
  },[])

  const Brand = allBrand?.find(brand => {
    return brand.id == id;
  });
  ...
  function getUsers() {
    ...
    setPhotoBase64(Brand?.brand_image)
}
const put='put'
const handleSubmit =(e) =>{
   e.preventDefault()
    ...
    const data ={ 
    brand_name:name,
         brand_image: photo== undefined? null : photoBase64,
       _method:put 
     };
    dispatch(editBrand({data,id}));
   console.log(data);
...
<Grid container spacing={2} sx={{margin: '10px'}}>
  ...
  <input
    ...
    />
     <br/>
    <img src={photoBase64||Brand?.brand_image} alt='profile image'  width="100px" height="100px"/>
    {/* ||adminDetail.photo */}
