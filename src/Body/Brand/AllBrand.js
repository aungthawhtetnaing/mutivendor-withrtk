...
export default function AllBrand() {
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'center',
      format: (id) => <div>
                            <Link to={`/dashboard/editbrand/${id}`} style={{textDecoration:'none',color:"black"}} >
                              <Button 
  ...
  function handleDelete(id){
    ...
    .then(willDelete => {
      if (willDelete) {
        fetch(`http://192.168.2.106:9999/api/admin/brand/${id}`,{
          method:'DELETE'
...
const rows = allBrand?.map(brand => createData(brand.id, brand.brand_name, brand.brand_image));
