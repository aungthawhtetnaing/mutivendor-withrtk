...
import { editState } from '../../../Redux/Components/ShippingArea/AllState/EditStateSlice';
...
const State = AllState?.find(Dist => {
  return Dist.id == id;
});

console.log("State id id",State);
  ...
  function getUsers() {
    ...
    setDistrictName(State?.state_name)
...
const handleSubmit =(e) =>{
   e.preventDefault()
    ...
    district_id:selectedDistrict==""?State?.district_id:selectedDistrict,
    state_name:districtName,
       _method:put 
     };
    dispatch(editState({data,id}));
   console.log(data);
};
...
useEffect(()=>{
  getUsers();
    ...
    console.log("selectedDivision value",selectedDivision);
  ...
  return (
    <div>
      ...
      <Grid item xs={12}>
            <Typography sx={{fontSize:"24px",fontWeight:"bold"}}>Edit state</Typography>
        </Grid>
        <Grid item xs={5}>
...
<Grid container spacing={2} sx={{margin: '10px'}}>
  ...
  <FormControl fullWidth>
    ...
    <NativeSelect
  defaultValue={State?.division_id}
  inputProps={{
        name,
  ...
  <FormControl fullWidth>
    ...
    <NativeSelect
  defaultValue={State?.district_id}
  inputProps={{
        name,
        id: 'uncontrolled-native',
      }}
