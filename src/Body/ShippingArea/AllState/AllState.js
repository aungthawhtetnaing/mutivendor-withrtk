...
function createData(name, code,district,state ,size) {
  const density = size;
  return { name, code,district,state, size };
}
...
export default function AllState() {
  ...
  const columns = [
    ...
    { id: 'state', label: 'State Name', minWidth: 100 },
   
    {
      ...
      align: 'center',
      format: (id) => <div>
                            <Link to={`/dashboard/editstate/${id}`} style={{textDecoration:'none',color:"black"}} >
                              <Button 
  ...
  function handleDelete(id){
    ...
    .then(willDelete => {
      if (willDelete) {
        fetch(`http://192.168.2.106:9999/api/admin/shipping/state/${id}`,{
          method:'DELETE'
...
const AllState=useSelector(state=>state.allState.state)
...
const rows = AllState?.map(state => createData(state.id, state.division,state.district,state.state_name ));
...
useEffect(()=>{
  ...
  return (
    ...
    <div>
      ...
      <Typography sx={{fontSize:"25px" ,fontWeight:"bold",marginBottom:"20px"}}>All State</Typography>
    <Grid item sx={12}>
    <Link to={`/dashboard/addstate`} style={{textDecoration:'none',color:"black"}} >
