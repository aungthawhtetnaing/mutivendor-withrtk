...
export default function AllCategory() {
  ...
  const columns = [
    ...
    { id: 'code', label: 'Brand Name', minWidth: 100 },
    ...
    },
    {
      ...
      align: 'center',
      format: (id) => <div>
                            <Link to={`/dashboard/editcategory/${id}`} style={{textDecoration:'none',color:"black"}} >
                              <Button 
  ...
  function handleDelete(id){
    ...
    .then(willDelete => {
      if (willDelete) {
        fetch(`http://192.168.2.106:9999/api/admin/category/${id}`,{
          method:'DELETE'
...
const rows = allCategory?.map(brand => createData(brand.id, brand.category_name, brand.category_image));
  ...
  return (
    ...
    <div>
      {/* <Link to="editbrand"  style={{textDecoration:'none',color:"black"}} > */}
                          ...
                          {/* </Link> */}
      <Typography sx={{fontSize:"25px" ,fontWeight:"bold",marginBottom:"20px"}}>All Category</Typography>
